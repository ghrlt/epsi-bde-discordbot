...
class OAuthInApp_Input_modal(
    discord.ui.Modal, title="Connexion à 360Learning | Identifiants"
):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        env.logger.debug(
            "%s#%s (%i) submited OAuthInApp form."
            % (
        ...
        if env.ASK_FOR_PASSWORD_ON_JOIN:
            ...
            try:
                ...
                await interaction.followup.send(
                    content="`❌` %s. Veuillez réessayer." % e, ephemeral=True
                )
                return
...
class OAuthInApp_NotStudent_modal(discord.ui.Modal, title="Vérification"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        env.logger.debug(
            "%s#%s (%i) submited OAuthInApp NotStudent form."
            % (
                interaction.user.name,
                interaction.user.discriminator,
                interaction.user.id,
        ...
        if not prenom or not nom or not email or not role:
            await interaction.response.send_message(
                content="`❌` Veuillez remplir tous les champs.", ephemeral=True
            )
            return
        ...
        try:
            await interaction.user.edit(
                nick="%s %s. | %s" % (prenom, nom.title()[0], role)
            )
        except discord.errors.Forbidden:
            env.logger.warning(
                "Guild: %i // Failed to rename user %s#%s (%i)."
...
class OAuthInApp_view(discord.ui.View):
    ...
    @discord.ui.button(
        label="Accéder au serveur",
        emoji="🤖",
        style=discord.ButtonStyle.primary,
        custom_id="in_app_oauth2_btn",
    )
    async def callback(
        ...
        env.logger.debug(
            "%s#%s (%i) clicked on OAuthInApp button."
            % (
                interaction.user.name,
                interaction.user.discriminator,
                interaction.user.id,
...
class OAuthInAppFailed_view(discord.ui.View):
    ...
    @discord.ui.button(
        ...
        emoji="🕵️",
        style=discord.ButtonStyle.primary,
        custom_id="in_app_oauth2_not_student_btn",
    )
    async def callback(
        self, interaction: discord.Interaction, button: discord.ui.Button
    ):
        env.logger.debug(
            "%s#%s (%i) clicked on OAuthInApp_NotStudent button."
            % (
                interaction.user.name,
                interaction.user.discriminator,
                interaction.user.id,
